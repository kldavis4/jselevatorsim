module.exports = function (id, floors, msgListener) {
    ...
    function resolveCallRequest(req) {
        ...
        //First check for elevators that are currently on the requested floor
        //and add them all to a collection
        //TODO confirm that iteration order will be the same for each elevator instance
        for (elevatorId in elevatorStates) {
            if (elevatorStates.hasOwnProperty(elevatorId)) {
        ...
        //If no elevators found, check for elevators that are moving towards
        //the requested floor
        ...
        //Finally if no elevators found, then pick the nearest ready elevator
        ...
        //Elect an elevator
        //TODO Check iteration order will be the same for each instance
        if (readyOnFloor.length > 0) {
